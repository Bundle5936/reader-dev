...
- 图片代理是唯一回源入口（`fetch_image`），**每跳（含重定向）DNS 解析后校验目标为公网地址**——私网/回环/链路本地默认拒绝。
- 可信自托管书源可通过 `READER_SSRF_ALLOW_PRIVATE_HOSTS` 配置精确的 `host[:port]` 例外（例如 `10.0.0.4:7822`）；这不是全局放行，且会同时作用于书源抓取和图片代理，部署时应只列必要目标。
- `SSRF_ALLOW_PRIVATE` 全局开关仍仅供测试代码使用，生产不会通过它关闭 SSRF。
